...
class ImageBrowser:UIView,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    
    //  Get HD thumbnail images agent
    var delegate :ImageBrowserDelegate?
    
    //  Bearing the view parent view view
    var bottomView:UIView!
    
    //  If let go preload picture
    var isShow: Bool!
    
    //  If there is no thumbnail drawings show the picture
    //  If they aren't in the picture is what also don't show
    var defaultImage: UIImage!
    
    //  How many pictures of the currently displayed
    var indexImage: Int!
    
    //  High-resolution image array
    var arrayImage: [String]!
    
    //  Pictures show the View
...
extension ImageBrowser{
    ...
    //  Initial Setting for the Collection View
    func  creatCollectionView(){
        ...
        collectionView = UICollectionView.init(frame: CGRect(x: 0,y: 0,width: ScreenWidth + imageInterval,height: ScreenHeight),collectionViewLayout: FowLayout)
        collectionView.allowsMultipleSelection = true
        collectionView.register(MomentCell.self, forCellWithReuseIdentifier: "cellId")
    ...
    override func layoutSubviews(){
        ...
        if (isShow == false) {
            if (self.indexImage == nil) {
                let altShow = UIAlertView.init(title: "Error",message: "Setting imageInterval property",delegate: nil,cancelButtonTitle: "OK")
                altShow.show()
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell{
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cellId", for: indexPath) as! MomentCell
        cell.bottomView = self.bottomView
        if ((self.delegate?.GetTheThumbnailImage(indexPath.row)) != nil) {
            cell.setImageWithURL(arrayImage[indexPath.row],placeholderImage:(self.delegate?.GetTheThumbnailImage(indexPath.row))!,defaultImage:self.defaultImage)
        }else{
            if (self.defaultImage == nil) {
                self.defaultImage = UIImage.init()
            }
            cell.setImageWithURL(arrayImage[indexPath.row],placeholderImage:self.defaultImage,defaultImage:self.defaultImage)
        }
        return cell
    }
...
@objc(MomentCell)
class MomentCell:UICollectionViewCell,UIScrollViewDelegate,UIActionSheetDelegate{
    
    static let cellId = "MomentCell"
    ...
    func creatUI(){
        ...
        let singleTap = UITapGestureRecognizer.init(target: self,action: #selector(self.oneTouch(_:)))
        let doubleTap = UITapGestureRecognizer.init(target: self,action: #selector(self.twoTouch(_:)))
        let longpressGesutre = UILongPressGestureRecognizer(target: self,action: #selector(self.handleLongpressGesture(_:)))
    ...
    internal func setImageWithURL(_ url:String, placeholderImage:UIImage, defaultImage:UIImage){
        ...
        BigImage.kf.setImage(with: URL.init(string: url),placeholder: defaultImage,options: [.cacheMemoryOnly],progressBlock: { (receivedSize, expectedSize) in
    ...
    @objc func handleLongpressGesture(_ sender : UILongPressGestureRecognizer){
        if (sender.state == .began) {
            ...
            let saveImageAlt = UIActionSheet.init(title: "Save picture to Photo Library", delegate: self, cancelButtonTitle: "Cancel", destructiveButtonTitle: "Save")
    ...
    func actionSheet(_ actionSheet: UIActionSheet, willDismissWithButtonIndex buttonIndex: Int){
        if buttonIndex == 0 {
            UIImageWriteToSavedPhotosAlbum(self.BigImage.image!,self,#selector(MomentCell.image(_:didFinishSavingWithError:contextInfo:)), nil)
        }
    }
    
    @objc func image(_ image: UIImage, didFinishSavingWithError error: NSError?, contextInfo: AnyObject) {
        if error != nil {
            let altShow =  UIAlertView.init(title: nil,message: "Save photo error/n\(String(describing: error?.description))",delegate: nil,cancelButtonTitle: "OK")
            altShow.show()
            return
        }
        setNotification(str: "Save photo succeed")
    }
